...
const create = (req, res) => {
    const {id,name, category_id, title, price, description } = req.body;
    ...
    ( id,name, category_id ,title ,image ,price ,description )
    VALUES (?,?, ?, ?, ?, ? ,?)
  `;
    const values = [id, name,category_id, title, image, price, description];
...
const edit = (req, res) => {
  ...
  db.query("SELECT * FROM products WHERE id = ?", [id], (err, rows) => {
    ...
    const sql = `
      UPDATE products
      SET name = ? , category_id=?, title=?, image=?, price=?, description=?
      WHERE id=?
    `;

    const values = [name ,category_id, title, image, price, description, id];
